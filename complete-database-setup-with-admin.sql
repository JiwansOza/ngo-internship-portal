-- =====================================================
-- COMPLETE DATABASE SETUP WITH ADMIN AUTHENTICATION
-- For new Supabase project: ugqweqtlmgtgipbuslpj
-- =====================================================
...
-- This will be created when the first admin user signs up
-- You can manually create admin users after they sign up by running:
...
AND table_name IN ('profiles', 'applications', 'onboarding_tasks', 'fundraising_progress', 'certificates', 'affiliate_links', 'donations')
ORDER BY table_name;
...
AND routine_name IN ('update_updated_at_column', 'generate_link_code', 'handle_new_user', 'create_default_affiliate_link', 'update_fundraising_on_donation', 'is_admin', 'get_all_fundraising_data')
...
SELECT id, name FROM storage.buckets WHERE id IN ('resumes', 'certificates');
...
-- ✅ All triggers for automatic operations
...
-- 2. Check if all data is created automatically
-- 3. Test affiliate links feature
-- 4. Make a user admin and test admin features 